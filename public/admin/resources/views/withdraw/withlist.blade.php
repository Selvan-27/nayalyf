                ...
                <table class="table table-responsive text-center" id="basic-1">
                    ...
                    <tbody>
                        ...
                        <td>{{ \Carbon\Carbon::parse($item->created_at)->format('d-m-Y') }}</td>
                           ...
                           <td>
    <input type="text" name="remake_{{$item->id}}" id="remake_{{$item->id}}">
</td>
<td>
    <div>
        <a href="/withdraw_update?wstatus=success&&id={{$item->id}}" 
        ...
        <a href="javascript:void(0);"  
           class="btn btn-secondary mt-md-0 mt-2" 
           onclick="return declineRequest({{$item->id}});">
           Decline
        </a>
    </div>
</td>
...
function declineRequest(id) {
    let remakeValue = document.getElementById('remake_' + id).value.trim();

    console.log(remakeValue);
    if (!confirm('Are you sure you want to decline?')) {
        return false;
    }

    // redirect with remake value
    let url = `/withdraw_update?wstatus=cancel&&id=${id}&&remake=${encodeURIComponent(remakeValue)}`;
