...
class activation_Controller extends Controller
{
    ...
    return back()->with('error', 'Only regular member IDs can be activated.');
      ...
      if(plan_activation_queue::where('activation_id', $request->Memberid)->exists())
      {
        ...
        $transaction->login_id = "ADMIN";
        $transaction->activation_id = $request->Memberid;
    ...
    if($transaction->save()){
        ...
        return back()->with('error', 'Invalid Error.');
         ...
         public function change_memberid(Request $request){
    
     $date = Carbon::now();
    $date= $date->format('Y-m-d');
   
       $new= $request->new_Memberid;
        $id=$request->Memberid;
        
    $plan = mlm_plan::where('memberid', $request->Memberid)->first();

    if (!$plan) {
    return back()->with('error1', 'Member ID does not exist.');
    }

    if ($plan->memberid_type !== 'regular') {
    return back()->with('error1', 'Only regular member IDs.');
    }
  ...
  $users = User::where('email',$id)->first();
   $users->update([
        ...
        'memberid' => 0,
         ...
         return back()->with('success1', 'Update memberid Successful.');
       ...
       return $transaction = new withdraw_history();

        $transaction->memberid = $login_id;
        $transaction->payout = $request->wallet_ID*.9;
        $transaction->service_charge = $request->wallet_ID*.01;
        $transaction->netpay = $request->wallet_ID*.2;
