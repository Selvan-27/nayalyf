...
class HomeController extends Controller
{
    ...
    public function product_details(Request $request){
     ...
     $item = Product::where('id', $id)->first();
        ...
        return view('ecom.product_details', compact('item'));
    ...
    $query = Product::where('type', 'products')
                    // ->where('is_active', 1)
                    ->where('id', '>', 5);
// return $request->cat;
if ($request->cat){
...
if ($request->d){
    ...
    switch ($sort) {
        ...
        case 'top': // Top-rated product
            $query->orderBy('rating', 'desc');
            break;

        case 'latest': // Latest arrival
        ...
        default:
            // Default sort (can be latest or none)
        ...
        return view('ecom.shop', compact('discount','products','categories','products_3','products_4'));
