...
class AuthController extends Controller
{
    ...
    $date = Carbon::now();
        ...
        $memberid= 'NL' . str_pad(mt_rand(1111111, 99999999), 7, '0', STR_PAD_LEFT); // Random ID with padding, total length 9 while (User::where('memberid', $memberid)->exists()) { $memberid = 'UC' . str_pad(mt_rand(1, 9999999), 7, '0', STR_PAD_LEFT); }
            
        while (User::where('memberid', $memberid)->exists()) 
        { 
            $memberid = 'NL' . str_pad(mt_rand(1111111, 99999999), 7, '0', STR_PAD_LEFT); 
